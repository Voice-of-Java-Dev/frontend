...
const Index = () => {
  ...
  const scrollToLatestPosts = () => latestPostsRef.current?.scrollIntoView({ behavior: 'smooth' });
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'dark' : ''}`}>
      <div className="bg-fixed bg-cover bg-center relative flex-grow" style={{ backgroundImage: `url(${import.meta.env.BASE_URL}java-bg.jpeg)` }}>
        ...
        <div className="relative z-10">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="relative hidden sm:flex items-center">
                      ...
                      {showSuggestions && suggestions.length > 0 && (
                        <ul className="absolute top-full left-0 w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md z-50 max-h-60 overflow-y-auto shadow-lg">
                          {suggestions.map(p => (
                            <li key={p._id} onMouseDown={() => handleSuggestionClick(p.slug, p._id)}
                              className="px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer">
                              <div className="font-semibold" dangerouslySetInnerHTML={{ __html: highlightText(p.title, debouncedQuery) }} />
                              <div className="text-sm text-gray-500">{(p.tags || []).map(tag => `#${tag}`).join(' ')}</div>
                            </li>
                          ))}
                        </ul>
                      )}
                    ...
                    {user ? (
                      <>
                        ...
                        <Button variant="ghost" size="sm" onClick={handleUserClick}><LogOut className="mr-1" /> Logout</Button>
                      </>
                    ) : (
                      <Button variant="ghost" size="sm" onClick={handleUserClick}><LogIn className="mr-1" /> Login</Button>
                    )}
            ...
            <section className="bg-gradient-to-br from-blue-600 to-blue-800 text-white py-20 mt-20">
              <div className="max-w-4xl mx-auto text-center px-4">
                ...
                <Button size="lg" variant="secondary" className="bg-white text-blue-600 hover:bg-gray-100" onClick={scrollToLatestPosts}>
                  Start Reading
                </Button>
              </div>
            </section>
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {postsToDisplay.map((post) => (
                      <PostCard key={post._id} post={post} onPostUpdated={fetchAllPosts} highlightQuery={debouncedQuery} />
                    ))}
                  </div>
                  {page < totalPages && (
                ...
                <div className="grid md:grid-cols-4 gap-8">
                  ...
                  <div>
                    ...
                    <div className="flex space-x-4 mt-2">
                      <a href="#" target="_blank" rel="noopener noreferrer"><Github className="w-5 h-5 hover:text-white text-gray-400" /></a>
                      <a href="#" target="_blank" rel="noopener noreferrer"><Linkedin className="w-5 h-5 hover:text-white text-gray-400" /></a>
                      <a href="#" target="_blank" rel="noopener noreferrer"><Instagram className="w-5 h-5 hover:text-white text-gray-400" /></a>
                    </div>
                  </div>
                </div>
                <div className="border-t border-gray-800 mt-8 pt-8 text-center text-gray-400">
                  <p>&copy; 2025 Voices of Java Dev. All rights reserved.</p>
                </div>
              </div>
            </footer>
